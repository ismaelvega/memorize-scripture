...
- Update Type or Speech mode: touch `components/type-mode-card.tsx` / `speech-mode-card.tsx`, adjust shared helpers (`lib/utils.ts`, `lib/audio-utils.ts`, `lib/storage.ts`) as needed, and verify history rendering.
