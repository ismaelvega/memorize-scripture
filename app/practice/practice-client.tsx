...
import type { ProgressListRow } from '@/lib/progress-rows';
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <React.Suspense fallback={<PracticeContentFallback />}>
        <PracticeContent 
          ...
          onCloseFlow={handleCloseFlow}
          initialRemoteRows={initialRemoteRows}
        />
      </React.Suspense>
    </div>
  );
...
function PracticeContent({ showFlow, setShowFlow, showSaved, setShowSaved, closingFlowRef, onCloseFlow, flowOrigin, setFlowOrigin, initialRemoteRows }: { showFlow: boolean; setShowFlow: (show: boolean) => void; showSaved: boolean; setShowSaved: (show: boolean) => void; closingFlowRef: React.MutableRefObject<boolean>; onCloseFlow: () => void; flowOrigin: 'browse' | 'progress' | 'saved' | null; setFlowOrigin: React.Dispatch<React.SetStateAction<'browse' | 'progress' | 'saved' | null>>; initialRemoteRows?: ProgressListRow[] }) {
  const router = useRouter();
  const searchParams = useSearchParams();
