...
import type { AppMode, Attempt, TrackingMode, Verse } from '@/lib/types';
import { sanitizeVerseText } from '@/lib/sanitize';
import { useNavigationWarning } from '@/lib/use-navigation-warning';
import { mapAttemptRows } from '@/lib/attempts-map';

interface PracticeModeClientProps {
  mode: string;
  initialRemoteAttempts?: Attempt[];
  initialRemoteAttemptsId?: string | null;
  initialRemoteFetched?: boolean;
}
...
export default function PracticeModeClient({
  mode,
  initialRemoteAttempts = [],
  initialRemoteAttemptsId = null,
  initialRemoteFetched = false,
  ...
  const [remoteAttempts, setRemoteAttempts] = React.useState<Attempt[]>(initialRemoteAttempts);
  const skipInitialFetchRef = React.useRef(Boolean(initialRemoteFetched && initialRemoteAttemptsId));
  ...
  React.useEffect(() => {
    ...
    async function loadRemoteAttempts() {
      ...
      if (skipInitialFetchRef.current && idParam === initialRemoteAttemptsId) {
        skipInitialFetchRef.current = false;
        return;
      }
      try {
        const res = await fetch(`/api/pull-attempts?verseId=${encodeURIComponent(idParam)}`);
        if (!res.ok) {
          setRemoteAttempts([]);
          return;
        }
        ...
        setRemoteAttempts(mapAttemptRows(data));
      } catch {
  ...
  }, [idParam, initialRemoteAttemptsId]);
