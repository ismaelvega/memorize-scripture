...
import { Card, CardHeader, CardTitle, CardDescription, CardContent, Button, Textarea, Badge, Progress, TooltipIconButton, Separator, Skeleton } from './ui/primitives';
    ...
    <Card className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-1" aria-label="Hint words">
              {[0,3,6].map(n => <Button key={n} size="sm" variant={hintWords===n? 'default':'outline'} onClick={()=>setHintWords(n)} disabled={!verse}>{n}</Button>)}
            </div>
      ...
      <CardContent className="flex flex-col gap-4 flex-1 overflow-auto">
        <div className="space-y-2">
          {hintWords>0 && verse && <p className="text-xs text-neutral-500 flex items-center gap-1"><Lightbulb size={14}/> Hint: <span className="font-mono">{hint}</span></p>}
